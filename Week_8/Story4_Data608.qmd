...
The yellow and orange areas represent neighborhoods with the highest number of Airbnb listings (1000-2000+).
The dark blue areas indicate fewer listings, suggesting less Airbnb activity in those neighborhoods.
High-Density Airbnb Areas

The highest concentration of listings appears in parts of Manhattan and Brooklyn, especially around Midtown Manhattan and Williamsburg.
These areas are prime Airbnb hotspots, likely due to strong demand from tourists and business travelers.
Low-Density Airbnb Areas
...
-   Visualizing Average Price for Listings with Premium Amenities
...
```
Key Insights:
Listings with Premium Amenities Charge Higher Prices

Gym, Pool, and Elevator significantly increase the price per night.
Listings with a Gym or Pool have the highest average price (~$350/night).
...
Visualizing Occupancy Rate for Listings with Premium Amenities
...
Let’s see this by neighborhood group:

## Minimum Nights vs Price by Neighborhood Group

```{r}
# Average minimum nights and price by neighborhood group
...
# Plot
ggplot(stay_summary, aes(x = reorder(neighbourhood_group, avg_price))) +
  geom_col(aes(y = avg_min_stay), fill = "skyblue", width = 0.6) +
  geom_line(aes(y = avg_price / 2), group = 1, color = "red", size = 1.2) +
  geom_point(aes(y = avg_price / 2), color = "red", size = 3) +
  scale_y_continuous(
    name = "Avg. Minimum Nights",
    sec.axis = sec_axis(~.*2, name = "Avg. Price ($)")
  ) +
  theme_minimal() +
  labs(
    title = "Minimum Stay vs Price by Neighbourhood Group",
    x = "Neighbourhood Group"
  )
...
1. Minimum Night Requirements Are Similar Across Boroughs
The average minimum stay requirement across all groups hovers around 25–30 nights, with little variation.
This indicates many hosts comply with NYC’s regulations that enforce minimum stay limits (especially in multi-unit buildings).

2. Price Varies Significantly by Area
...
-   Since budget travelers look for affordability, let’s compare room types in lower-priced areas (Bronx, Queens, Staten Island).
-   Room Type Distribution in Lower-Priced Areas
...
## Superhost vs. Regular Host: Do They Charge More?

-   Superhosts often have higher occupancy & pricing power. Let's compare pricing and revenue per night.
-   Superhost vs. Regular Host Pricing & Revenue
